...
def place_order(request):
    if request.method == 'GET':
        ...
        return render(request,'place_order.html',{'carts':carts,'total_price':total_price,'nums':nums,'user_address':user_address})
...
def order(request):
    if request.method =='POST':
        ...
        # 4. 下单完成后 删除购物车内容
        shop_cart.delete() #删除数据库购物车内容

        session_goods = request.session.get('goods')
        for se_goods in session_goods[:]:
            #se_goods 结构为 [goods_id,nums ,is_select]
            if se_goods[2]:
